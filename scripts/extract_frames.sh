...
for i in {1..6}; do
  if [ -f "public/videos/v${i}.mp4" ]; then
    echo "Processing v${i}.mp4 -> 1080p JPEG sequence (15fps, max 120 frames)..."
    # -y overwrites. scale=1920:-1 keeps aspect ratio. fps=15. -vframes 120 caps at 120 frames (8 seconds).
    ffmpeg -y -i "public/videos/v${i}.mp4" -vf "scale=1920:-1,fps=15" -vframes 120 -q:v 4 "public/sequence/v${i}/frame_%d.jpg"
  else
